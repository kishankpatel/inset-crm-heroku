 ...
 def after_destroy(record)   
    if record.class.name == "Task"
        ...
        record_status = (!record.taskable.nil? || !record.taskable.blank?) ?  (!record.taskable.is_public.nil? ? record.taskable.is_public : false) : false
         Activity.create(:organization_id => org_id,	:activity_user_id => activity_user_id,:activity_type=> activity_type, :activity_id => activity_id, :activity_status => activity_status,:activity_desc=>activity_desc,:activity_date => Time.zone.now, :is_public => record_status, :source_id => record.taskable.id,:source_type=>record.taskable.class.name)
    end
  end
