...
require 'will_paginate/array' 
  ...
  def search_all
    ...
    @deals = @current_organization.deals.where("title like ? or contact_info like ?","%#{query}%", "%#{query}%" )
    
    @individual_contacts = @current_organization.individual_contacts.where("first_name like ? or last_name like ? or email like ?", "%#{query}%", "%#{query}%", "%#{query}%")
    
    @tasks = @current_organization.tasks.where("title like ?", "%#{query}%" )

    @projects = @current_organization.projects.where("name like ? OR short_name like ?", "%#{query}%", "%#{query}%" )
