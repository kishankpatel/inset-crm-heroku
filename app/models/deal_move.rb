...
class DealMove < ActiveRecord::Base
  ...
  def insert_deal_move_activity
     Activity.create(:organization_id => self.organization_id,  :activity_user_id => self.user.id,:activity_type=> "DealMove", :activity_id => self.id, :activity_status => "Move",:activity_desc=>self.deal.title,:activity_date => self.updated_at, :is_public => (self.deal.is_public.nil? ||  self.deal.is_public.blank?) ? false : self.deal.is_public, :source_id => self.deal.id,:source_type=>self.deal.class.name)
  end  
