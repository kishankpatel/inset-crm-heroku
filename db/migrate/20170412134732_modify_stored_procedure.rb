class ModifyStoredProcedure < ActiveRecord::Migration
  def up
  	 execute <<-__EOI
    ...
    execute <<-__EOI
      ...
      BEGIN
          ...
          select organization_id into vr_organization_id from users  where id = vr_current_user limit 1;
          ...
          select count(deals.id) into cnt_existing_source from deals  inner join notes on notes.notable_id = deals.id and notes.notable_type='Deal'
              ...
              and notes.notes = vr_description limit 1;
          if(cnt_existing_source = 0) then
          select id into vr_assigned_to from users where email=vr_assigned_user limit 1;
          select id into vr_priority_type_id from priority_types where original_id = (case  vr_priority when 'hot' then 1 when 'warm' then 2 when 'cold' then 3 end ) and organization_id = vr_organization_id limit 1;
          ...
          if (vr_company_contact_id =0 ) then
            select id into vr_company_strength_id from company_strengths where `range` = replace(replace(vr_company_size,'(',''),')','') limit 1;
       ...
       select count(*) into vr_won_deals from deals where deal_status_id = vr_won_id and organization_id = vr_organization_id and id IN (select deal_id from deals_contacts where contactable_type = 'IndividualContact' and contactable_id = vr_individual_contact_id ) limit 1;
      ...
      select id, vr_deal_id, 'Deal','tags',curdate(),vr_organization_id,'Organization'  from tags where name in (select item from Tempsplitvalues) limit 1;
