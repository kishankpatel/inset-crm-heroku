class UpdateActivities
  ...
  def perform(activity)    
    ...
    if activity_classname == "Task"
     ...
     if record.present?
           a1 = Activity.create(:organization_id => org_id,	:activity_user_id => activity_user_id,:activity_type=> activity_type, :activity_id => activity_id, :activity_status => activity_status,:activity_desc=>activity_desc,:activity_date => activity_date, :is_public => record_status, :source_id => record.id,:source_type=>record.class.name)
