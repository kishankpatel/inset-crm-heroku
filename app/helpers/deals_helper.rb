module DealsHelper
  COLORS = ["#4D4DFF", "#FFA500", "#4D7865", "#808000", "#4F4F4F", "#506987", "#517B58", "#551011", "#551A8B", "#5E5E5E", "#615E3F", "#6600FF", "#A52A2A", "#FF00FF", "#34282C"]
...
# end
  ...
  def initialize_zendesk_api_client(api_integration)
    ZendeskAPI::Client.new do |config|

      config.url = api_integration.url

      # Basic / Token Authentication
      config.username = api_integration.account_id

      # Choose one of the following depending on your authentication choice
      config.token = api_integration.auth_token
      config.retry = true
  ...
  def get_my_deals_count org, current_user
    org.deals.joins(:deals_contacts).where("deals.is_active = true and ((deals.assigned_to = #{current_user.id} or deals.initiated_by = #{current_user.id} ) or (deals_contacts.contactable_type = 'IndividualContact' and deals_contacts.contactable_id in (select id from individual_contacts where owner_id = #{current_user.id})))").count
  end
